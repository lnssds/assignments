...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        points = points3D_xyz.unsqueeze(0)  # (1, N, 3)
        # Use torch.cdist instead of pytorch3d knn_points (avoid extra dependency)
        dists = torch.cdist(points, points) ** 2  # squared Euclidean, (1, N, N)
        dists, _ = torch.topk(dists, K, dim=-1, largest=False)  # (1, N, K) nearest
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # S is diagonal, so S = Sᵀ, and S @ Sᵀ = S² = diag(scales²)
